...
def mainLogin(firstCheck, driver, getIndate, client):
    if firstCheck == 0:
...
#    print (html)
    ...
    checkDate = 0    #발주관리 리스트에 날짜가 있는지 확인한다
    ...
    if checkDate == 0 :
        newFile = client.copy("1LWE7V5L-9OjNn8dydc2kMWVTxjM3r0qz9A9svoo2pHE",title=getIndate,  copy_permissions=True, folder_id='12gSLxwXRhMjoV2ekMlCp1YVmR4JqUpZv' )
        newFile.share('temper82@gmail.com', perm_type='user', role='owner')
        targetDateUrl = newFile.url + "/edit#gid=0"
        while True:
            try :           ## 발주서 관리 시트에 날짜와 URL 기입하기
                manageSheet.insert_row([getIndate, targetDateUrl], bulletinLastLine)
                break
            except : continue

        print("발주관리에 해당 날짜, 파일생성완료")
...
def run_main(getDate):
    # 발주 관리 리스트
    # https://docs.google.com/spreadsheets/d/1MFc8cukd7Cir3ZpUl27MfIj1m3iEc7UiypDx3nsYpAA/edit#gid=0

    api = '/var/services/homes/colaa/pangcode/autoMachine/gspread.json'
    ...
    while (1):
        ...
        # else:
        #    print("날짜를 다시 입력하세요")
        # getIndate = '2021-08-14'
        getIndate = str(getDate)
        chkString = re.compile("\d{4}[\s-]?\d{2}[-]?\d{2}")
        chkString = chkString.findall(getIndate)
    ...
    while True :
        try :
            ...
            driver = webdriver.Chrome(chrome_options=options, executable_path="/var/services/homes/colaa/pangcode/chromedriver")
            print ('서버에 크롬 브라우져 실행 성공했습니다.')
            break
        except :
            print ('서버에 크롬 브라우져 실행 실패! 다시 실행합니다.')
            continue


    firstCheck = 0

    mainLogin(firstCheck, driver, getIndate,client)
    # while True :
    #     try :
